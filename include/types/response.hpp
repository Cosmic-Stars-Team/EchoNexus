...
namespace echo::type {
    ...
    constexpr std::string_view get_status_message(
        unsigned int status
    ) {
        switch (status) {
        // --- 1xx Informational ---
    ...
    struct response {
        ...
        template <typename T>
            requires(!std::is_same_v<std::decay_t<T>, std::string> && !std::is_same_v<std::decay_t<T>, const char*>)
        static response json(
            T data,
            unsigned int code = 200
        ) {
